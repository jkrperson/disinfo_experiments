...
class FakeTLDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        if stage == "fit":
            self.fakenews_train = FakeTLDataset(self.data_dir + "/" + "train.csv", self.tokenizer)
            self.fakenews_valid = FakeTLDataset(self.data_dir + "/" + "test.csv", self.tokenizer) # There's no validation dataset apparently

        # Assign test dataset for use in dataloader(s)
        if stage == "test":
            self.fakenews_test = FakeTLDataset(self.data_dir + "/" + "test.csv", self.tokenizer)
